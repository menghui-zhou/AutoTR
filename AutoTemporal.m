...
%% Code starts here
function [W, R, out_list, funcVal] = AutoTemporal(X, Y, lambda1, lambda2, lambda3,  Auto_opts)
% Automatic Temporal Smoothness in Multi-task Learning

% Objective Function
% argmin_{W, R} { sum_i^m (0.5 * norm (Y{i} - X{i}' * W(:, i))^2)
%            + rho1 * \|W - WR\|_F^2 + rho2 * \|R .* S\|_1
...
while num_it <= Auto_opts.maxIter
    %     if mod(num_it, 10) == 0
    %         fprintf('AutoTR:   %d-th iteration \n', num_it);
    %     end

    % fix R, update W
    W_old = W;
    [W, W_funcVal] = update_W(X, Y, R, lambda1, W_opts); %#ok<ASGLU>
    W_change = norm(W_old - W, 'fro')^2 / norm(W_old, 'fro')^2;
    W_change_list = cat(1, W_change_list, W_change);

    % fix W, update R
    R_old = R;
    [R, R_funcVal] = update_R(W, lambda1, lambda2, S, R_opts); %#ok<ASGLU>
...
% private function value
    function funcVal = Get_funcVal(W, R)
        ...
        funcVal = LW + FirstTerm + SecondTerm;
